...
function proc_appendage_benyuan(appendage)
{
	...
	local PHYSICAL_ATTACK_PERCENT = sqrChr.sq_GetLevelData(skill_index, 51, skill_level);//����%
	local MAGICAL_ATTACK_PERCENT = sqrChr.sq_GetLevelData(skill_index, 52, skill_level);//���%
	local MAGICAL_DEFENSE_PERCENT = sqrChr.sq_GetLevelData(skill_index, 53, skill_level);//����%
	local PHYSICAL_DEFENSE_PERCENT = sqrChr.sq_GetLevelData(skill_index, 54, skill_level);//����%
	local PHYSICAL_ATTACK = sqrChr.sq_GetLevelData(skill_index, 71, skill_level);//����
	local MAGICAL_ATTACK = sqrChr.sq_GetLevelData(skill_index, 72, skill_level);//���
	local MAGICAL_DEFENSE = sqrChr.sq_GetLevelData(skill_index, 73, skill_level);//����
	local PHYSICAL_DEFENSE = sqrChr.sq_GetLevelData(skill_index, 74, skill_level);//����

	local ADDITIONAL_MAGICAL_GENUINE_ATTACK_PERCENT = sqrChr.sq_GetLevelData(skill_index, 55, skill_level);//Լءڪ����̪��%
	local ADDITIONAL_PHYSICAL_GENUINE_ATTACK_PERCENT = sqrChr.sq_GetLevelData(skill_index, 56, skill_level);//Լءت����̪��%
	local ADDITIONAL_MAGICAL_GENUINE_ATTACK = sqrChr.sq_GetLevelData(skill_index, 75, skill_level);//Լءڪ����̪��
	local ADDITIONAL_PHYSICAL_GENUINE_ATTACK = sqrChr.sq_GetLevelData(skill_index, 76, skill_level);//Լءت����̪��

	local PHYSICAL_ATTACK_BONUS = sqrChr.sq_GetLevelData(skill_index, 57, skill_level);//ڪ����̪��%
	local MAGICAL_ATTACK_BONUS = sqrChr.sq_GetLevelData(skill_index, 58, skill_level);//ت����̪��%
	local COOLTIME_DECLINE = sqrChr.sq_GetLevelData(skill_index, 59, skill_level);//��?����
	COOLTIME_DECLINE = COOLTIME_DECLINE * -1; //1��100��?����0.1��
	local EQUIPMENT_PHYSICAL_ATTACK = sqrChr.sq_GetLevelData(skill_index, 60, skill_level);//ڪ����̪��
	local EQUIPMENT_MAGICAL_ATTACK = sqrChr.sq_GetLevelData(skill_index, 61, skill_level);//ت����̪��

	local EQUIPMENT_PHYSICAL_DEFENSE_PERCENT = sqrChr.sq_GetLevelData(skill_index, 81, skill_level);//ڪ��������%
	local EQUIPMENT_MAGICAL_DEFENSE_PERCENT = sqrChr.sq_GetLevelData(skill_index, 82, skill_level);//ت��������%
	local EQUIPMENT_PHYSICAL_DEFENSE = sqrChr.sq_GetLevelData(skill_index, 83, skill_level);//ڪ��������
	local EQUIPMENT_MAGICAL_DEFENSE = sqrChr.sq_GetLevelData(skill_index, 84, skill_level);//ت��������

	local PHYSICAL_CRITICAL_HIT_RATE = sqrChr.sq_GetLevelData(skill_index, 10, skill_level);//ڪ����̪
	local MAGICAL_CRITICAL_HIT_RATE = sqrChr.sq_GetLevelData(skill_index, 11, skill_level);//ت����̪

	local ATTACK_SPEED = sqrChr.sq_GetLevelData(skill_index, 12, skill_level);// ����
	local CAST_SPEED = sqrChr.sq_GetLevelData(skill_index, 13, skill_level);// �ۯ����
	local MOVE_SPEED = sqrChr.sq_GetLevelData(skill_index, 14, skill_level);//���

	local ELEMENT_ATTACK_FIRE_PERCENT = sqrChr.sq_GetLevelData(skill_index, 62, skill_level);//��%
	local ELEMENT_ATTACK_WATER_PERCENT = sqrChr.sq_GetLevelData(skill_index, 63, skill_level);//?%
	local ELEMENT_ATTACK_DARK_PERCENT = sqrChr.sq_GetLevelData(skill_index, 64, skill_level);//��%
	local ELEMENT_ATTACK_LIGHT_PERCENT = sqrChr.sq_GetLevelData(skill_index, 65, skill_level);//��%
	local ELEMENT_ATTACK_FIRE = sqrChr.sq_GetLevelData(skill_index, 77, skill_level);//��
	local ELEMENT_ATTACK_WATER = sqrChr.sq_GetLevelData(skill_index, 78, skill_level);//?
	local ELEMENT_ATTACK_DARK = sqrChr.sq_GetLevelData(skill_index, 79, skill_level);//��
	local ELEMENT_ATTACK_LIGHT = sqrChr.sq_GetLevelData(skill_index, 80, skill_level);//��

	local FIRE_RESISTANCE = sqrChr.sq_GetLevelData(skill_index, 41, skill_level);//����
	local WATER_RESISTANCE = sqrChr.sq_GetLevelData(skill_index, 42, skill_level);//?��
	local DARK_RESISTANCE = sqrChr.sq_GetLevelData(skill_index, 43, skill_level);//����
	local LIGHT_RESISTANCE = sqrChr.sq_GetLevelData(skill_index, 44, skill_level);//����

	local PHYSICAL_CRITICAL_DAMAGE_RATE = sqrChr.sq_GetLevelData(skill_index, 16, skill_level);//ڪ����̪߿��
	local MAGICAL_CRITICAL_DAMAGE_RATE = sqrChr.sq_GetLevelData(skill_index, 17, skill_level);//ت����̪߿��
	...
	local HIT_RECOVERY = sqrChr.sq_GetLevelData(skill_index, 70, skill_level);//����
