...
{
	if (!obj) return false;
	local isUse = obj.sq_IsUseSkill(163);
	if (isUse) 
	{
...
{
	if(!obj) return;
			local skill_level = sq_GetSkillLevel(obj, 163);

			local appendage = CNSquirrelAppendage.sq_AppendAppendage(obj, obj, 163, false,
			 "character/mage/goddessofwar/ap_goddessofwar.nut", false);

			appendage.setAppendCauseSkill(BUFF_CAUSE_SKILL, sq_getJob(obj), 163, skill_level);
		
			CNSquirrelAppendage.sq_AppendAppendageID(appendage, obj, obj, 163, true);
			...
			if(appendage)
			{
					...
					if(change_appendage)
					{
						...
						local CRITICAL_DAMAGE_RATE = obj.sq_GetLevelData(163, 1, skill_level);
