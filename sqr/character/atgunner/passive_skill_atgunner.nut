function ProcPassiveSkill_ATGunner(obj, skill_index, skill_level) {
	...
	case SKILL_ATGUNNER_CROSSBOWMASTERY:
		if (skill_level > 0) {
			...
			if (appendage && (sqrChr.getWeaponSubType() == 4)) {
				local level_crossbowmastery = sq_GetSkillLevel(obj, SKILL_ATGUNNER_CROSSBOWMASTERY);
				local TYPE_STUCK = -(level_crossbowmastery / 2)
				local EQUIPMENT_PHYSICAL_ATTACK = level_crossbowmastery
				local EQUIPMENT_MAGICAL_ATTACK = level_crossbowmastery
				local MAGICAL_CRITICAL_HIT_RATE = level_crossbowmastery / 2
				local ATTACK_SPEED = level_crossbowmastery * 2

				local change_appendage = appendage.sq_getChangeStatus("crossbowmastery");
				if (!change_appendage) {
					change_appendage = appendage.sq_AddChangeStatusAppendageID(obj, obj, 0, CHANGE_STATUS_TYPE_STUCK, false, TYPE_STUCK, APID_COMMON);
					change_appendage = appendage.sq_AddChangeStatusAppendageID(obj, obj, 0, CHANGE_STATUS_TYPE_EQUIPMENT_PHYSICAL_ATTACK, true, EQUIPMENT_PHYSICAL_ATTACK, APID_COMMON);
					change_appendage = appendage.sq_AddChangeStatusAppendageID(obj, obj, 0, CHANGE_STATUS_TYPE_EQUIPMENT_MAGICAL_ATTACK, true, EQUIPMENT_MAGICAL_ATTACK, APID_COMMON);
					change_appendage = appendage.sq_AddChangeStatusAppendageID(obj, obj, 0, CHANGE_STATUS_TYPE_MAGICAL_CRITICAL_HIT_RATE, false, MAGICAL_CRITICAL_HIT_RATE, APID_COMMON);
					change_appendage = appendage.sq_AddChangeStatusAppendageID(obj, obj, 0, CHANGE_STATUS_TYPE_ATTACK_SPEED, true, ATTACK_SPEED, APID_COMMON);
				}
				if (change_appendage) {
					change_appendage.clearParameter();
					change_appendage.addParameter(CHANGE_STATUS_TYPE_STUCK, false, TYPE_STUCK.tofloat());
					change_appendage.addParameter(CHANGE_STATUS_TYPE_EQUIPMENT_PHYSICAL_ATTACK, true, EQUIPMENT_PHYSICAL_ATTACK.tofloat());
					change_appendage.addParameter(CHANGE_STATUS_TYPE_EQUIPMENT_MAGICAL_ATTACK, true, EQUIPMENT_MAGICAL_ATTACK.tofloat());
	...
	case SKILL_BENYUAN:
		...
		break;
	case SKILL_QIANG_REN_GAI_LIANG:
		if(skill_level > 0) {
...
function onStartGhost_My_atgunner_stylish(zLNgEXMd8NOL) {
	if (CNSquirrelAppendage.sq_IsAppendAppendage(zLNgEXMd8NOL, "character/atgunner/appendage/ap_stylish.nut")) CNSquirrelAppendage.sq_RemoveAppendage(zLNgEXMd8NOL, "character/atgunner/appendage/ap_stylish.nut");
	...
	if (szPELYvN6WprwP) {
		...
		if (!fqpAOeyuYqkwoGeIlb) fqpAOeyuYqkwoGeIlb = szPELYvN6WprwP.sq_AddChangeStatusAppendageID(zLNgEXMd8NOL, zLNgEXMd8NOL, 0, CHANGE_STATUS_TYPE_HP_MAX, false, 0, APID_COMMON);
...
function isSubSkill_atgunner_KILLPOINT(etTHKCUHt3oHaObu) {
	...
	for (local XfxTJd8nJzWVwCuPBGkH1 = 0; XfxTJd8nJzWVwCuPBGkH1 < RLAK3Iq8eZ; XfxTJd8nJzWVwCuPBGkH1++) {
		...
		if (MVnBFMtsS5P5IaBI4O && isSameObject(etTHKCUHt3oHaObu, MVnBFMtsS5P5IaBI4O.getSource()) && MVnBFMtsS5P5IaBI4O.isValid()) {
			...
			if (d37RtunETwe != -1 && d37RtunETwe == Mp3mmlw7bfIJba6eDWs) M5rrAxW8LFsrntPG_4bV36.push(1);
			else M5rrAxW8LFsrntPG_4bV36.push(2);
