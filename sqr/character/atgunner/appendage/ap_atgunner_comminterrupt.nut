...
function SetSkillState(obj, skillindex, state, Arr) {
	...
	if (nowstate == state) return false;
	if (obj.sq_IsUseSkill(skillindex)) {
		if (nowstate == 19 || nowstate == 8 || nowstate == 22 || nowstate == 23 || nowstate == 20 || nowstate == 24 || nowstate == 37 || nowstate == 41 || nowstate == 27 || nowstate == 58 || nowstate == 236 || nowstate == 238 || nowstate == 239) atgunner_skill_huashi(obj);
		obj.sq_IntVectClear();
		if (Arr.len() < 1) {
...
function atgunner_skill_huashi(obj) {
	if (STATE_ATGUNNER_HUASHI < 5) STATE_ATGUNNER_HUASHI++;
	...
	appendage.sq_SetValidTime(sq_GetLevelData(obj, SKILL_ATGUNNER_HUASHI, 6, skill_level));
...
function atgunner_comminterrupt(appendage) {
	...
	case 4:
		...
		EnableSoften(obj, 48, 31);
		SetSkillState(obj, 48, 31, [0]);
		EnableSoften(obj, 109, 59);
