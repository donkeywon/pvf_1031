...
{
	appendage.sq_AddFunctionName("proc", "proc_appendage_rosedance_buff")
	appendage.sq_AddFunctionName("onStart", "onStart_appendage_rosedance_buff")
	appendage.sq_AddFunctionName("onEnd", "onEnd_appendage_rosedance_buff")
}
...
function onStart_appendage_rosedance_buff(appendage)
{
	if(!appendage) return;
	...
	local PHYSICAL_CRITICAL_DAMAGE_RATE = sq_GetLevelData(obj, SKILL_ROSEDANCE, 0, skill_level);
	local SPEED = sq_GetLevelData(obj, SKILL_ROSEDANCE, 1, skill_level);
	local change_appendage = appendage.sq_getChangeStatus("rosedance_buff");
	...
	sq_SetParameterOcularSpectrum(spectrumAppendage, 1000, 100, true, sq_RGBA(72, 36, 44, 255), sq_RGBA(72, 36, 44, 255), 2, 2, 2);
	sq_SetParameterOcularSpectrum(spectrumAppendage, 1000, 100, true, sq_RGBA(72, 36, 44, 255), sq_RGBA(72, 36, 44, 255), 2, 0, 1);
...
function proc_appendage_rosedance_buff(appendage)
{
	if(!appendage) return;
}
...
function onEnd_appendage_rosedance_buff(appendage)
{
	if (!appendage) return;
