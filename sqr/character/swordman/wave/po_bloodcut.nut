function setCustomData_po_bloodcut(obj, receiveData) {
	...
	if (id == 125) {
		...
		local ani = obj.getVar().GetAnimationMap("", "passiveobject/zrr_skill/newswordman/animation/icewave.ani");
		...
		local particleCreater = obj.sq_var.GetparticleCreaterMap("IceWaveEx", "passiveobject/character/swordman/particle/icewaveexiceexplosionparticle1.ptl", obj);
...
function onAttack_po_bloodcut(obj, damager, boundingBox, isStuck) {
	...
	if (!CNSquirrelAppendage.sq_IsAppendAppendage(damager, result)) {
		local masterAppendage = CNSquirrelAppendage.sq_AppendAppendage(damager, obj, 100, false, result, true);
...
function procAppend_po_bloodcut(obj) {
	...
	if (id == 125) {
		...
		if (currentT >= 10 && obj.getVar("flag").get_vector(0) == 0) {
			...
			sq_createAttackObjectWithPath(obj, "passiveobject/character/swordman/animation/icewaveexiceexplosion.ani", "passiveobject/character/swordman/attackinfo/icewaveexiceexplosion.atk", false, atk2, size, 0, 0, 37);
