function ProcPassiveSkill_Gunner(obj, skill_index, skill_level) {
	if (skill_index == SKILL_BAO_TOU_ZHANG_WO)
	{
		if (skill_level > 0)
		{
	...
	case SKILL_CROSSBOWMASTERY:
		if (skill_level > 0) {
			...
			if (appendage && (sqrChr.getWeaponSubType() == 4)) {
				local level_crossbowmastery = sq_GetSkillLevel(obj, SKILL_CROSSBOWMASTERY);
...
function onStartGhost_My_gunner_stylish(M79WZ6Po87Wh_ewXh) {
	if (CNSquirrelAppendage.sq_IsAppendAppendage(M79WZ6Po87Wh_ewXh, "character/gunner/appendage/ap_stylish.nut")) CNSquirrelAppendage.sq_RemoveAppendage(M79WZ6Po87Wh_ewXh, "character/gunner/appendage/ap_stylish.nut");
	...
	if (KmIn77O8eYUVgTzl31AAH9Q) {
		...
		if (!wZAX3ErPBBb9q9U2UMgbBDAG) wZAX3ErPBBb9q9U2UMgbBDAG = KmIn77O8eYUVgTzl31AAH9Q.sq_AddChangeStatusAppendageID(M79WZ6Po87Wh_ewXh, M79WZ6Po87Wh_ewXh, 0, CHANGE_STATUS_TYPE_HP_MAX, false, 0, APID_COMMON);
